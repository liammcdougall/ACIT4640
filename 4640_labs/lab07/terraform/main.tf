...
# Create a list of random number the size of the number of ec2 instances to configure
# this will be used to randomly select an AMI for each instance
# Uses the `random_integer` resource from the `random` provider
# Reference: https://registry.terraform.io/providers/hashicorp/random/latest/docs/resources/integer
resource "random_integer" "ami_index" {
  min   = 0
  max   = 1
  count = length(var.instance_configs)
}

# create list of AMI ids for different OS's
locals {
  # generate a list of AMI ids and names
  ami_list = [
    { id   = var.ubuntu_ami_id
      name = "ubuntu"
    },
    { id   = var.centos_ami_id,
      name = "centos"
    }
  ]
  # create a list of numbers either 0 or 1 to randomly select an AMI
  ami_index = [for num in random_integer.ami_index : num.result]
}
...
resource "aws_instance" "main" {
  ...
  ami = local.ami_list[local.ami_index[count.index]].id
  ...
  tags = {
    ...
    OS = local.ami_list[local.ami_index[count.index]].name
  }
